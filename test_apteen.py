...
def test_apteen():
    ...
    while len(apteen.alive_non_sinks) > 0:
        ...
        # print(apteen.route)
        # apteen.plot()  # Commented to avoid generating many files
    ...
    ax.set_xlabel("round")
    ax.set_ylabel("number of alive nodes")
    plot_path = f"{results_dir}/test_apteen.png"
    ...
    if SNAPSHOT_GIF and snapshot_paths:
        if imageio is None:
            ...
            gif_path = f"{results_dir}/test_apteen.gif"
            try:
                frames = []
                target_size = None
    ...
    if TOPO_GIF and topo_paths:
        if imageio is None:
            ...
            gif_path = f"{results_dir}/topology_apteen.gif"
            try:
                frames = []
                target_size = None
                for p in topo_paths:
